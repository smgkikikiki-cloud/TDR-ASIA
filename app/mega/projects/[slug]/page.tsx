...
import { getMegaProject, getMegaEvents, getMegaProjectParticipants, getMegaProjectPackages, fmtThb } from '@/lib/mega-store';
...
export default async function ProjectDetail({params}:{params:Promise<{slug:string}>}){
  ...
  if(!p)notFound();
  const [events,participants,packages]=await Promise.all([getMegaEvents(100),getMegaProjectParticipants(p.id),getMegaProjectPackages(p.id)]);
  ...
  return <>
    ...
    <section className="megaSection"><div className="megaShell megaDetailGrid">
      <div>
        ...
        <div className="megaPanel megaParticipants"><div className="megaSectionHead"><h2>Work packages</h2><span className="megaMuted">Verified procurement scope</span></div>{packages.length?<div className="megaParticipantList">{packages.map(item=><div className="megaParticipant" key={item.id}><div><div className="megaMeta"><span className="megaPill">{item.recordType.replaceAll('_',' ')}</span><span className="megaPill">{item.status.replaceAll('_',' ')}</span></div><b>{item.packageCode} · {item.name}</b><small>{[item.category,item.procurementMethod].filter(Boolean).join(' · ')||'Package details pending'}</small></div><div className="megaParticipantSource">{item.valueThb!=null?<strong>{fmtThb(item.valueThb)}</strong>:null}{item.sourceUrl?<a href={item.sourceUrl} target="_blank" rel="noreferrer">Source ↗</a>:null}{item.verifiedAt?<span>Verified {new Date(item.verifiedAt).toLocaleDateString('en-GB')}</span>:null}</div></div>)}</div>:<div className="megaNotice">No verified work packages yet. TDR Mega will not infer package or tender records from the project name alone.</div>}</div>
